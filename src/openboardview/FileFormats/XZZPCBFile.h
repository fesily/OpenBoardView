...
struct XZZPCBFile : public BRDFileBase {
  ...
  private:
	...
	void parse_via_block(const std::vector<char> &buf);
	...
	void parse_text_block(const std::vector<char> &buf);
	...
	void apply_net(std::string &net, int &netId, uint32_t net_index);
